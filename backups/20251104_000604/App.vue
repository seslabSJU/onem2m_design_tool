...
export default {
  ...
  methods: {
    ...
    async deleteResourceFromServer(element) {
      ...
      const resourcePath = element.fullPath;
      if (!resourcePath) {
        throw new Error('리소스 경로를 계산할 수 없습니다. RN을 확인하세요.');
      }

      let finalPath = resourcePath;
      if (target.basePath && target.basePath.length > 0 && !resourcePath.startsWith(target.basePath)) {
        finalPath = `${target.basePath}/${resourcePath}`;
      }

      finalPath = finalPath.replace(/\/+/g, '/');

      try {
        console.log('[DELETE] TinyIoT resource:', finalPath);
        await http_resource_delete(this.originator, target.protocol, target.host, target.port, finalPath);
      } catch (error) {
        if (error?.response?.status === 404) {
          console.warn('Resource already absent on server:', finalPath);
        } else {
          throw error;
        }
      }
...
async loadResources() { 
    ...
    convertListToFileFormat(data) {
        ...
        const convertedData = {
            ...
            attrs: rootAttrs,
            selected: false,
            createdOnServer: true  // 서버에서 로드한 CSE 리소스이므로 이미 서버에 존재
        };
